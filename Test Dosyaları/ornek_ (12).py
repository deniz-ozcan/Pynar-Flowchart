...
def main():
    shape = int(
        input(
            "Kare için 1, 2 for rectangle, 3 for circle, 4 for triangle, 5 for cylinder, 6 for cone, or 7 for sphere: "
        )
    )
    if shape == 1:
        side = float(input("Kenar zunluk giriniz: "))
        print("Area of square = " + str(side ** 2))
    elif shape == 2:
        l = float(input("Uzunluk giriniz: "))
        b = float(input("Enter breadth: "))
        print("Area of rectangle = " + str(l * b))
    elif shape == 3:
        r = float(input("Çap giriniz:: "))
